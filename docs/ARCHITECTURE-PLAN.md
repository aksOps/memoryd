...
| `local` *(shipped 2026-06-10)* | **in-process** bge-small-en-v1.5 fp32 ONNX via tract (pure Rust, model `include_bytes!`-embedded, SHA-256 pinned at build) | `no_additional_spend` | **yes (default)** | embed (384-dim, real semantic signal), chat→none | The "best quality + consistency, no consumer overhead" embedding path: no network, no provider drift, air-gap safe. Queries use the bge instruction prefix (`embed_query`). Evidence: SciFact fused R@10 0.851 vs lexical 0.784. |
