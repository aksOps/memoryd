...
`memoryd integrate` auto-discovers installed AI coding agents and registers the
memoryd MCP server into each (Claude Code also gets a SessionEnd hook that runs
a `dream` pass to consolidate the session's captures):

```bash
memoryd integrate                 # all detected agents, user scope
